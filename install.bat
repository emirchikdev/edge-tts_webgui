...
:: Проверка, что conda вообще доступна после активации
where conda >nul 2>&1
if %errorlevel% neq 0 (
...
echo.
set "answer="
set /p answer="Создать обычное виртуальное окружение Python (venv)? (Y/N): "
...
:: ИСПРАВЛЕНИЕ: собираем пути отдельно, без массива внутри for
...
if !count! == 0 (
    ...
    echo Установи Python 3.10+ и отметь "Add Python to PATH", либо поставь Miniconda.
    echo.
    pause
    exit /b
)
...
:CHOOSE_LOOP
set "user_choice="
set /p user_choice="Выберите номер версии Python (1-!count!): "

:: ИСПРАВЛЕНИЕ: защита от нечислового ввода
echo !user_choice!| findstr /r "^[0-9][0-9]*$" >nul 2>&1
if !errorlevel! neq 0 (
    echo [ВНИМАНИЕ] Введите число от 1 до !count!.
    goto CHOOSE_LOOP
)
if !user_choice! LSS 1 goto CHOOSE_LOOP
if !user_choice! GTR !count! goto CHOOSE_LOOP

:: ИСПРАВЛЕНИЕ: корректное разыменование через call set
...
echo [INFO] Создаем виртуальное окружение в папке .venv...
